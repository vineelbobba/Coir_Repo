...
namespace Coir_ERP.Tests
{
    public abstract class Coir_ERPTestBase : AbpIntegratedTestBase<Coir_ERPTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected Coir_ERPTestBase()
        {
        ...
        protected void UsingDbContext(Action<Coir_ERPDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<Coir_ERPDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<Coir_ERPDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<Coir_ERPDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<Coir_ERPDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Coir_ERPDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<Coir_ERPDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Coir_ERPDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<Coir_ERPDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<Coir_ERPDbContext, Task<T>> func)
        {
            T result;
